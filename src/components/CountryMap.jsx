...
function LeafletMap({ geoData, nameKey, center, zoom, hoverColor, onRegionClick, selectedRegion }) {
  ...
  return (
    <MapContainer
      ...
      keyboard={false}
      boxZoom={false}
      style={{ width: '100%', height: '100%', background: countryId === 'maroc' ? '#DDD0BB' : '#FFFFFF' }}
    >
...
export default function CountryMap({ countryId: cid, continent }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-[#E8E0CC] dark:border-[#30363D]">
      ...
      <div className="relative" style={{ height: 'max(70vh, 560px)', background: cid === 'maroc' ? '#DDD0BB' : '#FFFFFF' }}>
        {loading && (
          <div className="absolute inset-0 flex items-center justify-center z-[1000]" style={{ background: cid === 'maroc' ? '#DDD0BB' : '#FFFFFF' }}>
